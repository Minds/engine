...
class ElggRiverItem
{
    ...
    private function addToTimelines()
    {
        ...
        if ($this->access_id == ACCESS_PRIVATE) {
            $followers = []; //don't add to followers timelines if this is private.
        }
        ...
        if ($this->subject->guid == elgg_get_logged_in_user_guid()) {
            array_push($followers, "personal:" . $this->subject->guid);
        }//add to their personal feed
        ...
        foreach ($followers as $follower_guid) {
            ...
            $db->insert($follower_guid, [$this->id => time()]);
        }
    }
    ...
    private function removeFromTimelines()
    {
        ...
        if ($this->action_type == 'feature') {
            $followers = [$this->action_type];
        }
        
        $db = new Minds\Core\Data\Call('timeline');
        foreach ($followers as $follower_guid) {
            if ($follower_guid) {
                $db->removeAttributes($follower_guid, [$this->id], false);
            }
        }
    }
