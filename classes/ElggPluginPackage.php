...
class ElggPluginPackage
{
    ...
    public function __construct($plugin, $validate = true)
    {
        ...
        if (strpos($plugin, $plugin_path) === 0 || is_dir($plugin)) {
            ...
            if (preg_match('/[^a-z0-9\.\-_]/i', $plugin)) {
                throw new PluginException(elgg_echo('PluginException:InvalidID', [$plugin]));
            }
        ...
        if (!is_dir($path)) {
            throw new PluginException(elgg_echo('PluginException:InvalidPath', [$path]));
        }
        ...
        if ($validate && !$this->isValid()) {
            if ($this->errorMsg) {
                ...
                throw new PluginException(elgg_echo('PluginException:InvalidPlugin', [$plugin]));
            }
        }
    ...
    private function isSaneDeps()
    {
        ...
        foreach ($provides as $provide) {
            ...
            if (!in_array($provide['type'], $this->providesSupportedTypes, true)) {
                $this->errorMsg =
            ...
            foreach (['conflicts', 'requires'] as $dep_type) {
                foreach (${$dep_type} as $dep) {
                    if (!in_array($dep['type'], $this->depsSupportedTypes, true)) {
                        $this->errorMsg =
