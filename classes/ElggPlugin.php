...
//			var_dump($PLUGINS_CACHE[$plugin], $plugin);
     ...
     * @return bool
     */
    public function save($timbased = true)
    {
        return true;
    }
    ...
    public function isValid()
    {
        if (!$this->getPackage() instanceof ElggPluginPackage) {
            echo 'no package';
            $this->errorMsg = elgg_echo('ElggPlugin:NoPluginPackagePackage', [$this->getID(), $this->guid]);
            return false;
        }
    ...
    protected function registerLanguages()
    {
        ...
        if (!register_translations($languages_path)) {
            $msg = elgg_echo(
                ...
                [$this->getID(), $this->guid, $languages_path]
            );
            throw new PluginException($msg);
        }
