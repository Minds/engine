...
class ElggAutoP
{
    ...
    protected function _addParagraphs(DOMElement $el)
    {
        ...
        while ($el = array_shift($elsToProcess)) {
            ...
            $alterInline = in_array($el->nodeName, $this->_alterList, true);
            ...
            while (null !== $node) {
                ...
                if ($isElement) {
                    $isBlock = in_array($node->nodeName, $this->_blocks, true);
                } else {
                    $isBlock = false;
                }
                ...
                if ($alterInline) {
                    ...
                    $isLastInline = (! $node->nextSibling
                                   || ($node->nextSibling->nodeType === XML_ELEMENT_NODE
                                       && in_array($node->nextSibling->nodeName, $this->_blocks, true)));
                    if ($isElement) {
                ...
                if ($isBlock || ! $node->nextSibling) {
                    if ($isBlock) {
                        if (in_array($node->nodeName, $this->_descendList, true)) {
                            $elsToProcess[] = $node;
