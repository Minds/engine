...
ArrayAccess // Override for array access
{
    ...
    {
        $this -> __iterator_valid = (false !== reset($this -> __attributes));
    }
    ...
    {
        $this -> __iterator_valid = (false !== next($this -> __attributes));
    }
    ...
    public function load($url)
    {
        ...
        $row = elggmulti_getdata_row(['domain' => $url]);
        if (!$row) {
            return false;
        }
    ...
    {
        return ['MultisiteDomain' => 'Elgg domain', ];
    }
}
...
function elggmulti_db_connect()
{
    ...
    if (empty($MULTI_DB -> pool)) {
        ...
        $cfs = ['mindsuser_to_domain', 'domain'];
...
function elggmulti_getdata($options, $callback = '')
{
    ...
    $defaults = ['type' => 'domain', 'limit' => 12, 'offset' => "", ];
...
function elggmulti_get_db_settings($url = '')
{
    ...
    $result = elggmulti_getdata_row(['domain' => $url], '__elggmulti_db_row');
...
function elggmulti_is_plugin_available($plugin)
{
    ...
    return in_array($plugin, $activated, true);
}
...
function elggmulti_get_installed_plugins()
{
    ...
    if ($handle = opendir($path)) {
        while ($mod = readdir($handle)) {
            if (!in_array($mod, ['.', '..', '.svn', 'CVS'], true) && is_dir($path . "/" . $mod)) {
...
{
    return [	'pluginmanager' => ['activated'=>true],
                    'minds' => ['activated'=>true],
                    'mindsmulti_pluginmanager' => ['activated'=>true],
                    'minds_themeconfig' => ['activated'=>true],
                    'minds_theme_selector' => ['activated'=>true],
