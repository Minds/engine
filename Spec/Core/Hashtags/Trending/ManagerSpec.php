...
class ManagerSpec extends ObjectBehavior
{
    public $repository;
    public $cache;
    private Collaborator $hashtagExclusionServiceMock;
    private Collaborator $configMock;

    public function let(
        Repository $repository,
        BasicCacheInterface $cache,
        HashtagExclusionService $hashtagExclusionServiceMock,
        Config $configMock,
    ) {
        $this->repository = $repository;
        $this->cache = $cache;
        $this->hashtagExclusionServiceMock = $hashtagExclusionServiceMock;
        $this->configMock = $configMock;
        $this->beConstructedWith($repository, $cache, $hashtagExclusionServiceMock, $configMock);
    }
    ...
    public function it_should_get_currently_trending_tags_from_repository_using_previous_days_trending(
        ...
        $this->hashtagExclusionServiceMock->getExcludedHashtags()
            ->shouldBeCalled()
    ...
    public function it_should_get_currently_trending_tags_from_repository_using_previous_days_trending_and_exclude_excluded_tags(
        Response $previouslyTrendingResponse,
        Response $currentlyTrendingResponse,
    ) {
        $previouslyTrendingTags = [
        ...
        $this->repository->getList(Argument::that(function ($opts) {
            return is_int($opts['from']) &&
                isset($opts['exclude_tags']) &&
                $opts['exclude_tags'] = ['hashtag1', 'hashtag2', 'excludedHashtag'];
        }))
            ->shouldBeCalled()
            ->willReturn($currentlyTrendingResponse);
