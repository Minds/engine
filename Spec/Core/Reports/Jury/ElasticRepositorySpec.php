...
class ElasticRepositorySpec extends ObjectBehavior
{
    ...
    public function let(Client $es, ReportsRepository $reportsRepository)
    {
    ...
    public function it_should_return_reports_we_can_use_in_appeal_jury()
    {
        $user = new User();
        $user->set('guid', 123);
        $user->setPhoneNumberHash('phoneHash');
    ...
    public function it_must_require_a_phone_hash()
    {
        $user = new User();
        $user->set('guid', 123);
    ...
    public function it_should_add_initial_jury_decision(Decision $decision)
    {
    ...
    public function it_should_add_initial_jury_decision_as_overturned(Decision $decision)
    {
        $ts = (int) round(microtime(true) * 1000);
        $this->es->request(Argument::that(function ($prepared) use ($ts) {
            $query = $prepared->build();
    ...
    public function it_should_add_appeal_jury_decision(Decision $decision)
    {
        $ts = (int) round(microtime(true) * 1000);
        $this->es->request(Argument::that(function ($prepared) use ($ts) {
            $query = $prepared->build();
    ...
    public function it_should_add_appeal_jury_decision_as_overturned(Decision $decision)
    {
        $ts = (int) round(microtime(true) * 1000);
        $this->es->request(Argument::that(function ($prepared) use ($ts) {
            $query = $prepared->build();
