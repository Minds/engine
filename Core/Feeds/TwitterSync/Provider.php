...
class Provider extends Di\Provider
{
    public function register()
    {
        $this->di->bind('Feeds\TwitterSync\Manager', function ($di) {
            return new Manager(new Client(), $di->get('Feeds\TwitterSync\Repository'), $di->get('Config'), $di->get('EntitiesBuilder'), new Save(), $di->get('Feeds\Activity\RichEmbed\Manager'), new Delegates\ChannelLinksDelegate($di->get('EntitiesBuilder')));
        });
