...
class DailyRewards
{
    ...
    public static function reward()
    {
        ...
        $row = $db->getRow("analytics:rewarded:day:$ts", ['offset'=>Core\Session::getLoggedinUser()->guid, 'limit'=>1]);
        if (!$row || key($row) != Core\Session::getLoggedinUser()->guid) {
            $db->insert("analytics:rewarded:day:$ts", [Core\Session::getLoggedinUser()->guid => time()]);
